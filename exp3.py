...
class FixTopoNetwork(Network):
    def build(self):
        ...
        n1: QNNode = self.make_node("n1")
        n2: QNNode = self.make_node("n2")
        n3: QNNode = self.make_node("n3")
        n4: QNNode = self.make_node("n4")
        n5: QNNode = self.make_node("n5")
        n6: QNNode = self.make_node("n6")
        n7: QNNode = self.make_node("n7")
        # n8: QNNode = self.make_node("n8")
...
class SelfTimer(Timer):
    def __init__(self, network, mode, start_time=0, end_time=30, step_time=0.01, alloc_time=1, name="t1"):
        # Call the parent constructor correctly
        super().__init__(name, start_time, end_time, step_time, trigger_func=None)
        self.network: FixTopoNetwork = network
        self.mode = mode  # Routing arm this trace belongs to
    ...
    def trigger(self):
        ...
        if self.last_write % 10 == 0:
            ...
            f.write(f"{self.mode},{self._simulator.current_time},{np.mean(self.l1)},{np.mean(self.l2)}\n")
            self.l1 = []
            self.l2 = []
...
def run(mode, reroute, predictive, w=10, m=10):
    ...
    for src in net.s:
        c = Counter([tuple(x.route) for x in src.sendedList])
