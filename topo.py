...
class Network(Entity):
    ...
    def __init__(self, n=10, p=0.5, reqs=3, memorySize=30, windowSize=100, queryTime=0.02, send_max_try=10,
                 ...
                 predictive=False, utility="ratio", hop_penalty=0.1, blend=0.7):
    ...
    def make_node(self, name) -> QNNode:
        """Build a node carrying this network's node configuration."""
        return QNNode(name, memorySize=self.memorySize, windowSize=self.windowSize,
                      queryTime=self.queryTime, send_max_try=self.send_max_try,
                      allow_reroute=self.allow_reroute, random_memory=self.random_memory,
                      predictive=self.predictive, utility=self.utility,
                      hop_penalty=self.hop_penalty, blend=self.blend)
    ...
    def build(self):
        ...
        for i in range(self.n):
            n: QNNode = self.make_node("n" + str(i + 1))
